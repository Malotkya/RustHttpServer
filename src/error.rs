...
#[derive(Debug, Clone)]
pub struct HttpError<E:fmt::Display>{
    error: E,
    kind: HttpErrorKind
}

impl<E:fmt::Display> HttpError<E>{
    pub fn new(kind: HttpErrorKind, error: E)-> HttpError<E>{
        Self{ kind, error }
    }

    pub fn message(&self)->String {
        let value = format!("{}", self.error);
        if value.len() == 0 {
            return String::from(self.kind.as_str());
        }
...
impl HttpErrorKind {
    ...
    //Client Errors
        ...
        return HttpErrorKind::BadRequeset;
    } else if code == 501 {
    ...
    pub fn as_str(&self)->&str {
        match self {
            ...
            //Server Error
            HttpErrorKind::BadRequeset => "BAD REQUEST",
